...
module.exports = (bot) => {
    ...
    bot.command("retry", async (ctx) => {
        try {
            ...
            const failed = (execData.data || []).filter(e => e.status === "error");
    ...
    // Store pending renames and their auto-expiry timers
    const pendingRenames = new Map();
    const pendingRenameTimers = new Map();
    ...
    bot.action(/^ops_rename_(.+)$/, async (ctx) => {
        try {
            ...
            // Auto-expire pending rename after 5 minutes to prevent memory leak
            const existingTimer = pendingRenameTimers.get(userId);
            if (existingTimer) clearTimeout(existingTimer);

            pendingRenames.set(userId, { workflowId: id, oldName: wf.name });

            const timer = setTimeout(() => {
                pendingRenames.delete(userId);
                pendingRenameTimers.delete(userId);
            }, 5 * 60 * 1000);
            pendingRenameTimers.set(userId, timer);
    ...
    bot.on("text", async (ctx, next) => {
        ...
        if (!newName || newName.startsWith("/")) {
            pendingRenames.delete(userId);
            clearTimeout(pendingRenameTimers.get(userId));
            pendingRenameTimers.delete(userId);
            return next();
        }
    ...
    bot.action(/^ops_webhook_(.+)$/, async (ctx) => {
        try {
            ...
            // Use the public-facing URL for webhook display, not the internal Docker network URL
            const baseUrl = process.env.WEBHOOK_URL || process.env.N8N_BASE_URL || "https://your-domain.com";
