...
const BACK_BTN = Markup.inlineKeyboard([[Markup.button.callback("◀️ Back", "help_back")]]);
    ...
    bot.action("help_backup", async (ctx) => {
        ...
        await ctx.editMessageText(
            [
                ...
                `/backup_system — Full system backup (DB + all data)`,
                `/daily_backup_on — Enable daily auto-backup (3 AM)`,
                `/daily_backup_off — Disable daily auto-backup`,
                `/daily_backup_status — Check schedule &amp; last run`,
                ``,
                `<b>Restore</b>`,
                `/restore_workflow — Upload &amp; restore a workflow backup`,
                `/restore_system — Full system restore (overwrite &amp; restart)`,
    ...
    bot.action("help_system", async (ctx) => {
        ...
        await ctx.editMessageText(
            [
                ...
                ``,
                `🔑 <b>Authentication</b>`,
                `/setkey &lt;key&gt; — Set n8n API Key (message auto-deleted)`,
                `/auth_status — Check API Key status`,
                `/debug — Full connectivity diagnostic`,
            ].join("\n"),
            { parse_mode: "HTML", ...BACK_BTN }
        );
    });
