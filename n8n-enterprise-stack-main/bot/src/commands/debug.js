...
module.exports = (bot) => {
    bot.command("debug", async (ctx) => {
        ...
        let hasKey = !!(apiKey && apiKey.length > 0);
        let hasBasic = !!(basicUser && config.n8n.pass);
        ...
        try {
            ...
            lines.push(`<b>Connection:</b> ❌ Unreachable (${escapeHtml(err.code || err.message)})`);
        }
        ...
        if (config.n8n.user && config.n8n.pass) {
            try {
                const authRes = await axios.get(`${baseURL}/rest/workflows`, {
                    auth: { username: config.n8n.user, password: config.n8n.pass },
                    timeout: 3000,
                    validateStatus: () => true
                });
                if (authRes.status === 200) {
                    lines.push("Basic Auth: ✅ Working (HTTP 200)");
                } else if (authRes.status === 401) {
                    lines.push(`Basic Auth: ❌ Rejected (HTTP 401 — Wrong credentials)`);
                } else {
                    lines.push(`Basic Auth: ⚠️ Unexpected status (HTTP ${authRes.status})`);
                }
            } catch (e) {
