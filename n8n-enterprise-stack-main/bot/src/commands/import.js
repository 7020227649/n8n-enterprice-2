 ...
 * If the buffer contains an array of workflows, imports each one.
 * Returns an array of result objects: { success, name, id?, error? }
 */
async function importSingleWorkflow(buffer, filename) {
    ...
    try {
        ...
        return [{ success: false, name: filename, error: "Invalid JSON: " + err.message }];
    }

    // n8n can export multiple workflows as a top-level array
    ...
    for (const item of items) {
        ...
        if (!workflow) {
            results.push({ success: false, name: filename, error: "Not a valid n8n workflow JSON" });
            continue;
        }
        try {
            ...
            console.error(`[Import] Failed to import ${workflow.name}:`, err.message);
            results.push({ success: false, name: workflow.name || filename, error: err.message });
        }
    }
    return results;
}
...
module.exports = (bot) => {
    ...
    bot.command("import", async (ctx) => {
        await ctx.reply(
            [
                ...
                ``,
                `├ <b>.json</b> — Single workflow <i>or</i> multiple workflows as a JSON array`,
                `└ <b>.zip</b>  — Multiple workflows (each as a .json file inside the archive)`,
                ``,
                `<i>💡 Tip: You can export a workflow from n8n and send the .json file directly — no ZIP needed.</i>`,
                ``,
    ...
    bot.on("document", async (ctx) => {
        ...
        try {
            ...
            if (isJson) {
                // ── Single JSON import (may contain multiple workflows as an array) ──
                const results = await importSingleWorkflow(buffer, doc.file_name);

                const succeeded = results.filter(r => r.success);
                const failed = results.filter(r => !r.success);

                if (results.length === 1 && succeeded.length === 1) {
                    // Single workflow — compact success message
                    const r = succeeded[0];
                    await ctx.reply(
                        [
                            ...
                            `├ ID: <code>${escapeHtml(String(r.id))}</code>`,
                            `└ Status: 🔴 Inactive`,
                            ``,
                ...
                } else if (results.length === 1 && failed.length === 1) {
                    // Single workflow — compact failure message
                    const r = failed[0];
                    await ctx.reply(
                        [
                            `❌ <b>Import Failed</b>`,
                    ...
                    // Multi-workflow JSON array
                    const lines = [
                        `📥 <b>JSON Import Complete</b>`,
                        ``,
