...
api.interceptors.request.use(async reqConfig => {
  ...
  try {
    ...
    // 2. Try State — state.get() handles decryption internally
    if (!apiKey) {
      apiKey = state.get("n8nApiKey");
    }

    // 3. Trim whatever we got
    if (apiKey) apiKey = apiKey.trim();

    // 4. Validate Key Length (200 char ceiling to reject obviously garbage values)
    if (apiKey && apiKey.length > 200) {
      console.warn(`[n8n API] Detected suspiciously long API Key (${apiKey.length} chars). Ignoring it.`);
      apiKey = null;
    }
...
module.exports = {
  ...
  /**
   * Get all workflows with FULL data (no validation/stripping).
   * Used by backup service to preserve all workflow fields.
   */
  async getAllWorkflowsRaw() {
    const res = await api.get("/workflows");
  ...
  /**
   * Get a single workflow with FULL data (no validation/stripping).
   * Used by backup service to preserve all workflow fields.
   */
  ...
  async activateWorkflow(id) {
    ...
    try {
      await api.post(`/workflows/${id}/activate`, {}, { baseURL: `${config.n8n.baseURL}/api/v1` });
      if (await this.verifyActive(id, true)) return await this.getWorkflow(id);
    } catch (e) { lastError = e; }
