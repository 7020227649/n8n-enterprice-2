...
async function exportAll() {
  ...
  const workflows = await n8nApi.getAllWorkflowsRaw();
...
async function exportSingleWorkflow(id) {
  ensureDir(BASE_DIR);
  const wf = await n8nApi.getWorkflowRaw(id);
  const date = new Date().toISOString().replace(/[:.]/g, "-");
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const filename = isMultipart
          ? `n8n-backup-${dateLabel}.zip.part${i + 1}`
          : `n8n-backup-${dateLabel}.zip`;
        await bot.telegram.sendDocument(chatId, { source: chunks[i], filename });
      }
